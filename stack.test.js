...
describe("push", function() {
  it("places the value at the top of the stack and returns undefined", function() {
    ...
    // ***not tracking with nodes, using array push&pop only to keep O(1)
    // expect(stack.first.val).toBe(10);
    // expect(stack.last.val).toBe(10);
    expect(stack.data[0]).toBe(10);
    expect(stack.data[stack.size-1]).toBe(10);
    stack.push(100);
    // expect(stack.first.val).toBe(100);
    // expect(stack.last.val).toBe(10);
    expect(stack.data[stack.size-1]).toBe(100);
    expect(stack.data[0]).toBe(10);
    stack.push(1000);
    // expect(stack.first.val).toBe(1000);
    // expect(stack.last.val).toBe(10);
    expect(stack.data[stack.size-1]).toBe(1000);
    expect(stack.data[0]).toBe(10);
  });
});
